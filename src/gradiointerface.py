...
def sampler_builder(timesteps, stop, stop_after_timesteps, n_sample, noise_type, sampler_type, model_filename, embeddings, gpu):
    ...
    sample(device, gpu_perf, timesteps, noise_type, n_sample, sampler_type, model_filename, context, grid_filename = './data/grid.png', stop_after_timesteps=stop_after_timesteps)
    return f"timesteps: {timesteps}, stop_before: {stop}, stop_after_timesteps: {stop_after_timesteps}, batch_size: {n_sample}, noise_type: {noise_type}, sampler_type: {sampler_type}, embeddings: {context}, gpu: {gpu}", "./data/grid.png"
...
demo = gr.Interface(
    ...
    [
        ...
        gr.Slider(1, 81, value=4, step=1, label="Batch size", info=""),
